...
public class IrodsextDataProfilerServiceTest {
	...
	public void testProxyDataProfileAsAdmin() throws Exception {
		...
		IrodsextDataTypeResolutionServiceFactoryImpl dataTyperFactory = new IrodsextDataTypeResolutionServiceFactoryImpl();
		dataTyperFactory.setDataTyperSettings(dataTyperSettings);
		dataTyperFactory.setIrodsAccessObjectFactory(accessObjectFactory);

		DataProfilerSettings proxySettings = new DataProfilerSettings();
		proxySettings.setDetectMimeAndInfoType(true);
		proxySettings.setRetrieveAcls(false);
		proxySettings.setRetrieveMetadata(true);
		proxySettings.setRetrieveReplicas(false);
		proxySettings.setRetrieveShared(false);
		proxySettings.setRetrieveStarred(false);
		proxySettings.setRetrieveTickets(false);

		IrodsextDataProfilerFactoryImpl dataProfilerFactory = new IrodsextDataProfilerFactoryImpl();
		dataProfilerFactory.setDataProfilerSettings(dataProfilerSettings);
		dataProfilerFactory.setDataTypeResolutionServiceFactory(dataTyperFactory);
		dataProfilerFactory.setIrodsAccessObjectFactory(accessObjectFactory);

		DataProfilerService dataProfilerService = dataProfilerFactory.instanceDataProfilerService(adminAccount,
				proxySettings);
		@SuppressWarnings("rawtypes")
		DataProfile dataProfile = dataProfilerService.retrieveDataProfile(dataName);
		Assert.assertNotNull("null data profile returned", dataProfile);
